    ...
    <footer id="footer" className="relative z-10 bg-(--brand-card) text-(--brand-ink) font-sans border-t border-(--brand-border) overflow-hidden">
      ...
      <div className="relative py-16 sm:py-24 px-6 sm:px-10 bg-linear-to-r from-(--brand-blue-deep) via-(--brand-blue) to-(--brand-green) text-white overflow-hidden">
        ...
        <div className="max-w-[1500px] mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 md:gap-10 text-center md:text-left">
          ...
          <div className="flex flex-col sm:flex-row gap-3.5 sm:gap-4 w-full sm:w-auto shrink-0">
            <Link 
              href="/driver-login" 
              className="w-full sm:w-auto px-9 py-4 sm:py-4.5 rounded-full text-xs font-black uppercase tracking-[0.18em] bg-white text-(--brand-ink) hover:bg-white/90 transition-all shadow-xl flex items-center justify-center"
            >
              Driver Portal →
            </Link>
            <Link 
              href="/owner-login" 
              className="w-full sm:w-auto px-9 py-4 sm:py-4.5 rounded-full text-xs font-black uppercase tracking-[0.18em] bg-white/20 backdrop-blur-md text-white hover:bg-white/30 border border-white/30 transition-all flex items-center justify-center"
            >
              Admin Center
            </Link>
          </div>
      ...
      {/* Main Footer Links */}
      <div className="max-w-[1500px] mx-auto px-6 sm:px-10 pt-16 sm:pt-20 pb-12 sm:pb-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 md:gap-12 lg:gap-8">
          
          {/* BRAND COLUMN */}
          <div className="lg:col-span-5 space-y-5 sm:space-y-6 pr-0 md:pr-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="font-mono text-[11px] sm:text-xs font-bold text-(--brand-blue-deep) tracking-widest uppercase">
