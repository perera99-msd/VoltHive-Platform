...
export default function MyGarage() {
  ...
  const executeAddVehicle = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToastMessage({ msg: 'Failed to add vehicle.', type: 'error' });
      }
    } catch (err) {
      setError('Network error');
      setToastMessage({ msg: 'Network error adding vehicle.', type: 'error' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleAddVehicle = (e: React.FormEvent) => {
    ...
    setPendingConfirm({
      title: 'Save EV Profile?',
      message: `Are you sure you want to add this ${make} ${model} to your EV garage?`,
      confirmText: 'Save EV Profile',
      action: executeAddVehicle
    });
  };
  ...
  const executeSetDefault = async (vehicleId: string) => {
    if (!user) return;
    try {
      const token = await user.getIdToken();
      ...
      if (res.ok) {
        const data = await res.json();
        setProfile(data);
        setToastMessage({ msg: 'Set as primary EV vehicle!', type: 'success' });
      } else {
        setToastMessage({ msg: 'Failed to set primary vehicle.', type: 'error' });
      }
    } catch (err) {
      console.error('Failed to set primary:', err);
      setToastMessage({ msg: 'Network error setting primary vehicle.', type: 'error' });
    }
  };

  const handleSetDefault = (vehicleId: string, vehicleName: string) => {
    setPendingConfirm({
      title: 'Set as Primary Vehicle?',
      message: `Set "${vehicleName}" as your default EV for map station compatibility filtering?`,
      confirmText: 'Set Primary',
      action: () => executeSetDefault(vehicleId)
    });
  };
    ...
    <section className="space-y-6 relative pb-32 md:pb-12 font-sans">
      ...
      >
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-semibold tracking-tight text-(--brand-ink) mt-1">
            My Garage &
            <span className="text-transparent bg-clip-text bg-linear-to-r from-(--brand-blue) to-(--brand-green)"> Vehicles</span>
          </h1>
        ...
        <button
          ...
          className="px-6 py-3 rounded-xl bg-linear-to-r from-(--brand-blue) to-(--brand-green) text-white font-bold text-sm shadow-md hover:brightness-105 active:scale-95 transition-all self-start md:self-auto shrink-0 cursor-pointer"
        >
          ＋ Add EV
        </button>
      </motion.header>
            ...
            >
                ...
                <button
                  onClick={() => {
                    setPendingConfirm({
                      ...
                      message: `Are you sure you want to remove "${v.make} ${v.model}" from your garage? All historical charging preferences for this EV will be purged.`,
                      confirmText: 'Remove Vehicle',
                      isDanger: true,
                      action: () => executeDeleteVehicle(v._id)
                    });
                  }}
              ...
              {!v.isPrimary && (
                <div className="mt-5 pt-4 border-t border-(--brand-border)/50 flex justify-end">
                  <button
                    onClick={() => handleSetDefault(v._id, `${v.make} ${v.model}`)}
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-(--brand-ink)/40 backdrop-blur-md">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-(--brand-card)/95 backdrop-blur-3xl border border-(--brand-border) rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-[0_34px_80px_-30px_rgba(9,32,52,0.7)] overflow-y-auto max-h-[90vh] text-(--brand-ink)"
            >
              ...
              <form onSubmit={handleAddVehicle} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" required placeholder="e.g. Tesla" value={make} onChange={e => setMake(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-(--brand-border) text-sm font-semibold bg-(--brand-card) text-(--brand-ink) focus:outline-none focus:border-(--brand-blue)" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-(--brand-muted) mb-1">Model</label>
                    <input type="text" required placeholder="e.g. Model 3" value={model} onChange={e => setModel(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-(--brand-border) text-sm font-semibold bg-(--brand-card) text-(--brand-ink) focus:outline-none focus:border-(--brand-blue)" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <input type="number" required placeholder="2024" value={year} onChange={e => setYear(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-(--brand-border) text-sm font-semibold bg-(--brand-card) text-(--brand-ink) focus:outline-none focus:border-(--brand-blue)" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-(--brand-muted) mb-1">Battery (kWh)</label>
                    <input type="number" required placeholder="60" value={batteryKWh} onChange={e => setBatteryKWh(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-(--brand-border) text-sm font-semibold bg-(--brand-card) text-(--brand-ink) focus:outline-none focus:border-(--brand-blue)" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-(--brand-muted) mb-1">Max DC (kW)</label>
                    <input type="number" required placeholder="150" value={maxKW} onChange={e => setMaxKW(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-(--brand-border) text-sm font-semibold bg-(--brand-card) text-(--brand-ink) focus:outline-none focus:border-(--brand-blue)" />
                  </div>
                </div>
